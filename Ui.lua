...
local Custom = {} do
  Custom.ColorRGB = Color3.fromRGB(187, 187, 187)
...
local vantablack_Library, Notification = {}, {}

vantablack_Library.Unloaded = false

function vantablack_Library:SetNotification(Config)
...
function vantablack_Library:CreateWindow(Config)
  local Title = Config[1] or Config.Title or ""
  local Description = Config[2] or Config.Description or ""
  local TabWidth = Config[3] or Config["Tab Width"] or 120
  local SizeUi = Config[4] or Config.SizeUi or UDim2.fromOffset(560, 400)

  local Funcs = {}

  local vantablackHubXGui = Custom:Create("ScreenGui", {
    ZIndexBehavior = Enum.ZIndexBehavior.Sibling
  }, RunService:IsStudio() and Player.PlayerGui or (gethui() or cloneref(game:GetService("CoreGui")) or game:GetService("CoreGui")))
  ...
  -- Background artwork fitted inside the menu
  local Background = Custom:Create("ImageLabel", {
    Image = "rbxassetid://71943760805011",
  ...
  Custom:Create("UICorner", {}, Background)

  -- Soft light so the artwork looks real and readable
  ...
  local Glow = Custom:Create("Frame", {
    BackgroundColor3 = Color3.fromRGB(255, 255, 255),
  ...
  Close.Activated:Connect(function()
    ...
    if vantablackHubXGui then vantablackHubXGui:Destroy() end
		if not vantablack_Library.Unloaded then vantablack_Library.Unloaded = true end
	end)
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      function Item:AddDropdown(Config)
        ...
        local SearchBar = Custom:Create("TextBox", {
          ...
          BackgroundColor3 = Color3.fromRGB(0, 0, 0),
          BackgroundTransparency = 0.9,
          BorderColor3 = Color3.fromRGB(189, 189, 189),
          BorderSizePixel = 1,
...
local Dis = "https://discord.gg/CrEXMRAxS"
...
vantablack_Library.FuncsV3 = FuncsV3
vantablack_Library.Version = Ver
vantablack_Library.Discord = Dis

return vantablack_Library
